...
MATERIAL_DIR = tcsv.MATERIAL_DIR
OUTPUT_DIR = tcsv.OUTPUT_DIR
...
print(Fore.GREEN+"""\n[INFO TACS]
[INFO USER] TACS version = """+version+""" 
...
db_prot = pd.read_csv(MATERIAL_DIR+'/Pmag_Prot.csv',index_col=0)
db_binaries = pd.read_csv(MATERIAL_DIR+'/Binaries_db.csv',index_col=0)
almanac_table = tcsf.load_nested_dict_npz(MATERIAL_DIR+'/almanac.npz')
db_starname = pd.read_csv(MATERIAL_DIR+'/THE_SIMBAD.csv',index_col=0)
...
table_time = pd.read_csv(MATERIAL_DIR+'/time_conversion.csv',index_col=0)
...
lightcurves = tcsf.load_nested_dict_npz(MATERIAL_DIR+'/Lightcurves.npz')
db_exoplanets = pd.read_csv(MATERIAL_DIR+'/exoplanets_db.csv',index_col=0)
...
db_tess_candidates = pd.read_csv(MATERIAL_DIR+'/TESS_candidates.csv',index_col=0)
...
db_stability = pd.read_csv(MATERIAL_DIR+'/StabilityCurves.csv',index_col=0)
...
def produce_gr8(version=None,verbose=False):
    ...
    gr8_raw = pd.read_csv(MATERIAL_DIR+'/THE_Master_table_v'+version+'.csv',index_col=0)
...
def get_info_starname(name,verbose=True):
    ...
    if button:
        ...
        if verbose:
            ...
            if len(planets):
                print('\n[INFO] Planets found:\n')
                display = planets[['name','method','year','period','mass','radius']].copy()
...
def plot_magcycle(starname,verbose=False,newfig=True,show_private=False):
    ...
    entries = []
    if index is not None:
        sun = pd.read_csv(MATERIAL_DIR+'/Sun_MG2.csv',index_col=0)
        the_mhk = pd.read_csv(MATERIAL_DIR+'/SNAKY_THE_FINCH_mag.csv',index_col=0)
...
def plot_ccf(starname,verbose=False,newfig=True,version=None):
    ...
    if index is not None:
        master = gr8[version]
        the_ccf = np.load(MATERIAL_DIR+'/SNAKY_THE_ccf.npy')
        the_infos = pd.read_csv(MATERIAL_DIR+'/SNAKY_THE_summary_infos.csv',index_col=0)
        entries = the_infos.loc[the_infos['star']==index['HD']]
        star_info = master.loc[np.where(master['HD']==index['HD'])[0]]

    if len(entries):
        sun_ccf = np.load(MATERIAL_DIR+'/SNAKY_Sun_ccf.npy')
...
def plot_spectrum(starname,verbose=False,newfig=True):
    ...
    entries = []
    if index is not None:
        the_spec = np.load(MATERIAL_DIR+'/SNAKY_THE_spec_6100_6200.npy')
        the_infos = pd.read_csv(MATERIAL_DIR+'/SNAKY_THE_summary_infos.csv',index_col=0)
        entries = the_infos.loc[the_infos['star']==index['HD']]
    
    if len(entries):
        sun_spec = np.load(MATERIAL_DIR+'/SNAKY_Sun_spec_6100_6200.npy')
...
class tcs(object):
    ...
    def union(self,selection1,selection2,union_name=None,figname=None,ordering='vmag', Xmarker={'under_review>':0.5}, extra='under_review'):
        ...
        union = gr8.loc[np.in1d(np.array(gr8['GAIA']),np.array(union['GAIA']))].copy()
